...
- (void)setMessage:(ChatCellMessage *)message
{
    ...
    if(_message.showAvatar) {
        ...
        UITapGestureRecognizer * ges = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(avatarTap1:)];
        ges.numberOfTapsRequired=1;
        [_avatarView addGestureRecognizer:ges];
    }
...
- (void)avatarTap1:(id)sender {
    if (self.chatCellDelegate) {
        [self.chatCellDelegate didAvatarTapedWithCell:self];
    }
}
